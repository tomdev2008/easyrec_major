...
//                    request.getScheme() + "://" + localName + ":" + request.getLocalPort() + webappPath;
            ...
            if(!file.delete())
                log("Failed to delete old api js file.");

            if(!tempFile.renameTo(file))
                log("Failed to move temp file to api js file.");
        } catch(Exception ex) {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content)
        ...
        if(requestedFile.equals("/easyrec.js") && request.getServletPath().equals("/api-js"))
            rewriteApiJs(request);
